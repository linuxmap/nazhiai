...
{
    LPDIRECT3DSURFACE9 surface = (LPDIRECT3DSURFACE9)frame->data[3];//这段功能是直接将解码后的数据显示出来,GPU使用率会提升
    InputStream        *ist = (InputStream *)s->opaque;
    DXVA2Context       *ctx = (DXVA2Context *)ist->hwaccel_ctx;
...
static int dxva2_alloc(AVCodecContext *s, HWND hwnd)
{
    ...
    do{
        ...
        if (!createDeviceManager) {
            printf("Failed to locate DXVA2CreateDirect3DDeviceManager9\n");
        ...
        if (!ctx->d3d9) {
            printf("Failed to create IDirect3D object\n");
        ...
        if (FAILED(hr)) {
            printf("Failed to create Direct3D device: %d\n", hr);
        ...
        if (FAILED(hr)) {
            printf("Failed to create Direct3D device manager\n");
        ...
        if (FAILED(hr)) {
            printf("Failed to bind Direct3D device to device manager\n");
        ...
        if (FAILED(hr)) {
            printf("Failed to create IDirectXVideoDecoderService\n");
        ...
        if (!ctx->tmp_frame) {
            printf("Failed to alloc context tmp frame\n");
            break;
        }

        s->hwaccel_context = av_mallocz(sizeof(struct dxva_context));
        if (!s->hwaccel_context) {
            printf("Failed to malloc hwaccel context\n");
            break;
        }
        return 0;
    }while(false);
...
int dxva2_init(AVCodecContext *s, HWND hwnd)
{
    ...
    if (!ist->hwaccel_ctx && hwnd != NULL) {
        ...
        if (ret < 0) {
            printf("failed to alloc dxva2: %d\n", ret);
            av_log(NULL, loglevel, "failed to alloc dxva2: %d\n", ret);
            return ret;
        }
    }
    ...
    if (s->codec_id == AV_CODEC_ID_H264 &&
        ...
        printf("Unsupported H.264 profile for DXVA2 HWAccel: %d\n", s->profile);
    ...
    if (ret < 0) {
        printf("Error creating the DXVA2 decod4er: %d\n", ret);
        av_log(NULL, loglevel, "Error creating the DXVA2 decoder: %d\n", ret);
        return ret;
    }
...
AvPacketBuffer::AvPacketBuffer(int capacity)
    : _bufferLocker(), _bufferCondition(), _buffer()
    , _capacity(capacity)
{
    AUTOLOCK(_bufferLocker);
    while (capacity-- > 0)
    {
        _buffer.push(av_packet_alloc());
    }
}
...
AVPacket* AvPacketBuffer::Alloc()
{
    WAIT_MILLISEC_TILL_COND(_bufferCondition, _bufferLocker, 1000, [this]{ return _buffer.size() > 0; });
    if (_buffer.size() > 0)
    {
        AVPacket* packet = _buffer.front();
        _buffer.pop();
        return packet;
    }
    else
...
void AvPacketBuffer::Free(AVPacket*& packet)
{
    if (packet)
    {
        av_packet_unref(packet);
        {
            AUTOLOCK(_bufferLocker);
            _buffer.push(packet);
            WAKEUP_ONE(_bufferCondition);
        }
        packet = nullptr;
    }
}

void AvPacketBuffer::Clear()
{
    AUTOLOCK(_bufferLocker);
    // free pop packet
    while (_buffer.size() > 0)
    {
        av_packet_free(&_buffer.front());
        _buffer.pop();
    }
}
